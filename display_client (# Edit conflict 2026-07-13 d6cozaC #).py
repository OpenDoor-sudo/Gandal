...
def translate_single_text(text, target_locale):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt == max_attempts - 1:
                print(f"[TRANSLATE TEXT ERROR] Gemini failed: {e}. Falling back to translate_via_llm...")
                return translate_via_llm(text, target_locale)
            time.sleep(1)
...
def generate_and_cache_dense_transcripts(vid_path, vid_id, locale="en_US"):
    ...
    try:
        ...
        for segment in transcript_data:
            ...
            if ts and text:
                # If target locale is French, translate text
    ...
    def do_GET(self):
        ...
        if clean_path in ['', '/', '/index.html', '/index']:
            ...
            if os.path.exists(index_file_path):
                ...
                # Resolve instructor name and locale dynamically from SQLite
                v_id = None
                if active_track and active_track.startswith("vid_"):
                    ...
                    v_id = "vid_economics_01"
                elif active_track in ["5th Grade", "College"]:
                    v_id = "vid_physics_01"
                else:
        ...
        if clean_path == '/api/lesson':
            ...
            # Save active video_id to active_session.json for the LiveKit agent process
            try:
                with open("active_session.json", "w") as f:
                    json.dump({"active_video_id": video_id}, f)
                print(f"[SESSION] Saved active video_id '{video_id}' to active_session.json")
            except Exception as e:
                print(f"[ERROR] Failed to save active_video_id: {e}")
            ...
            if (not timestamps or not has_transcripts) and video_id and resolved_video_path:
                ...
                threading.Thread(
                    ...
                    daemon=True
                                ...
                                conn_th.close()
                            if th_locale == "fr_FR":
                                print("[BG QWEN TIMESTAMPS] Active locale is fr_FR. Translating generated timestamps/cards to French...")
                                broad_ts = translate_timestamps_to_french(generated_ts)
                                broad_fc = translate_flashcards_to_french(generated_fc)
                        except Exception as th_err:
                            print(f"[BG QWEN TIMESTAMPS ERROR] Failed to translate: {th_err}")
                        ...
                        # Use a temporary event loop in this thread to send WebSocket broadcast
                        async def send_broadcast():
                            try:
                                async with websockets.connect(f"ws://localhost:{WS_PORT}") as ws:
                                    await ws.send(json.dumps(ws_payload))
                                    print(f"[BG QWEN TIMESTAMPS] WS broadcast complete for {vid_id}.")
                            except Exception as ws_err:
                                print(f"[BG QWEN TIMESTAMPS ERROR] WebSocket broadcast failed: {ws_err}")
                        ...
                        print(f"[BG QWEN TIMESTAMPS ERROR] Thread failed for {vid_id}: {err}")
